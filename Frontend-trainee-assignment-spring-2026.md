...
- TypeScript
- Использование предоставленного backend API (документация о нём написана ниже)
...
- Характеристики товара (зависят от категории, например для одежды: предмет одежды, цвет, размер, бренд)
- Полное описание
...
- Внешний вид: несколько URL изображений (необязательное)
...
- Кнопка «Улучшить описание» - отправляет данные объявления в LLM и подставляет улучшенный вариант в поле описания
...
- Установите зависимости, выполнив `npm i`
- Запустите сервер - `npm start`

Посе этого сервер будет запущен на порту 8080. Если необходимо запустить его на другом порту, то это можно сделать при помощи переменной окружения `PORT` - `PORT=8080 npm start`
...
- `?needsRevision={true | 1}` - фильтрация объявлений, требующих доработки
